...
export default function Dashboard() {
  ...
  const completedDeliveries = todayDeliveries.filter(
    (d) => d.status === "delivered",
  );
  const activeDeliveries = todayDeliveries.filter(
    (d) => d.status !== "delivered" && d.status !== "failed",
  );
  const distanceCovered = completedDeliveries.reduce(
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg, rgba(59,130,246,0.1) 0%, rgba(139,92,246,0.06) 40%, rgba(6,182,212,0.04) 100%)",
            ...
            allCompleted.slice(0, 4).map((delivery, idx) => (
              <div
                ...
                style={{
                  padding: "12px 16px",
                  borderBottom:
                    idx < Math.min(allCompleted.length, 4) - 1
                      ? "1px solid var(--border-soft)"
                      : "none",
              ...
              >
                ...
                <div className="flex flex-col flex-1 gap-0.5 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      {getRelativeTime(delivery.scheduledAt)}
                    </span>
                  </div>
                  <p
                    className="truncate"
